...
describe('isValidDate rule', () => {
  it('should return true if the value is a valid Date', () => {
    expect(isValidDate().check(mockContext({ value: '2020/04/14' }))).toBeTruthy();
    expect(isValidDate().check(mockContext({ value: new Date() }))).toBeTruthy();
  });

  it('should return false if the value is not a valid Date', () => {
    expect(isValidDate().check(mockContext({ value: 'foo' }))).toBeFalsy();
    expect(isValidDate().check(mockContext({ value: 'June 32, 2020' }))).toBeFalsy();
