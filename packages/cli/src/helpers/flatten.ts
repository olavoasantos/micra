...
export const flatten = (obj: Record<string, any>, separator: string = '.', normalizeKey: (key: string) => string = kebabCase) => {
    ...
    (toReturn: Record<string | number, string | number>, [name, value]) => {
      if (Array.isArray(value)) {
        ...
        Object.entries(flatten(value, separator, normalizeKey)).forEach(([key, definition]) => {
          toReturn[`${normalizeKey(name)}${separator}${normalizeKey(key)}`] = definition as string | number;
        });
      } else {
